...
func TestBinaryQueueIteratorFirst(t *testing.T) {
	queue := NewWith[int](utils.IntComparator)
	it := queue.Iterator()
	if actualValue := it.First(); actualValue != false {
...
func TestBinaryQueueIteratorLast(t *testing.T) {
	...
	it := tree.Iterator()
	if actualValue := it.Last(); actualValue != false {
		t.Errorf("Got %v expected %v", actualValue, false)
	}
	tree.Enqueue(2)
	tree.Enqueue(3)
...
func TestBinaryQueueSerialization(t *testing.T) {
	...
	newQueue := NewWith[int](utils.IntComparator)
	err = json.Unmarshal([]byte(`[1,2,3]`), &newQueue)
	if err != nil {
		t.Errorf("Got error %v", err)
	}
}
