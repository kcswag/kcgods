...
func TestMapRemove(t *testing.T) {
	...
	if actualValue, expectedValue := m.Keys(), []int{3, 4, 1, 2}; !sameElements[int](actualValue, expectedValue) {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	...
	if actualValue, expectedValue := fmt.Sprintf("%s", containers.ToInterfaceSlice(m.Keys())), "[]"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	if actualValue, expectedValue := fmt.Sprintf("%s", containers.ToInterfaceSlice(m.Values())), "[]"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
...
func assertSerialization[K, V string](m *Map[K, V], txt string, t *testing.T) {
...
func benchmarkPut[V struct{}](b *testing.B, m *Map[int, V], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Put(n, struct{}{})
		}
...
func benchmarkRemove[V comparable](b *testing.B, m *Map[int, V], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Remove(n)
		}
