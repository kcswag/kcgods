...
func TestAVLTreePut(t *testing.T) {
	...
	if actualValue, expectedValue := fmt.Sprintf("%d%d%d%d%d%d%d", containers.ToInterfaceSlice(tree.Keys())...), "1234567"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s%s%s%s%s", containers.ToInterfaceSlice(tree.Values())...), "abcdefg"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
...
func TestAVLTreeRemove(t *testing.T) {
	...
	if actualValue, expectedValue := fmt.Sprintf("%d%d%d%d", containers.ToInterfaceSlice(tree.Keys())...), "1234"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s%s", containers.ToInterfaceSlice(tree.Values())...), "abcd"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s%s", containers.ToInterfaceSlice(tree.Values())...), "abcd"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	...
	if actualValue, expectedValue := fmt.Sprintf("%s", containers.ToInterfaceSlice(tree.Keys())), "[]"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
...
func TestAVLTreeSerialization(t *testing.T) {
	...
	newTree := NewWithStringComparator[int]()
	err = json.Unmarshal([]byte(`{"a":1,"b":2}`), &newTree)
	if err != nil {
		t.Errorf("Got error %v", err)
	}
}
