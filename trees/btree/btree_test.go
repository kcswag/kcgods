...
func TestBTreeRemove7(t *testing.T) {
	...
	// use simulator: https://www.cs.usfca.edu/~galles/visualization/BTree.html
	tree := NewWithIntComparator[float64](3)
	tree.Put(1, 0.0)
	tree.Put(2, 0.0)
	tree.Put(3, 0.0)
	tree.Put(4, 0.0)
...
func TestBTreeIteratorValuesAndKeys(t *testing.T) {
	...
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s%s%s%s%s", containers.ToInterfaceSlice(tree.Values())...), "xbcdefg"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	if actualValue := tree.Size(); actualValue != 7 {
		t.Errorf("Got %v expected %v", actualValue, 7)
...
func assertValidTree[K, V comparable](t *testing.T, tree *Tree[K, V], expectedSize int) {
...
func assertValidTreeNode[K, V comparable](t *testing.T, node *Node[K, V], expectedEntries int, expectedChildren int, keys []K, hasParent bool) {
...
func TestBTreeSerialization(t *testing.T) {
	...
	newTree := NewWithStringComparator[int](3)
