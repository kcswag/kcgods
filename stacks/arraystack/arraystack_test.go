...
func TestStackSerialization(t *testing.T) {
	...
	var err error
	assert := func() {
		if actualValue, expectedValue := fmt.Sprintf("%s%s%s", containers.ToInterfaceSlice(stack.Values())...), "cba"; actualValue != expectedValue {
			t.Errorf("Got %v expected %v", actualValue, expectedValue)
		}
	...
	newStack := New[int]()
	err = json.Unmarshal([]byte(`[1,2,3]`), &newStack)
	if err != nil {
		t.Errorf("Got error %v", err)
	}
}
