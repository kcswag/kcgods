...
func TestSetMap(t *testing.T) {
	...
	if actualValue := mappedSet.Contains("mapped: c", "mapped: b", "mapped: a"); actualValue != true {
		t.Errorf("Got %v expected %v", actualValue, true)
	}
	if actualValue := mappedSet.Contains("mapped: c", "mapped: b", "mapped: x"); actualValue != false {
		t.Errorf("Got %v expected %v", actualValue, false)
	}
	if mappedSet.Size() != 3 {
...
func TestSetAny(t *testing.T) {
	set := New[string]()
	set.Add("c", "a", "b")
	anyElement := set.Any(func(index int, value string) bool {
		return value == "c"
	})
	if anyElement != true {
...
func TestSetSerialization(t *testing.T) {
	...
	intSet := New[int]()
	err = json.Unmarshal([]byte(`[1,2,3]`), &intSet)
	if err != nil {
		t.Errorf("Got error %v", err)
	}
}
